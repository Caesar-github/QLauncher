...
Item {
    ...
    property int minimizedHeight: Math.round(48 * DisplayConfig.dp)
    property int minimizedWidth: Math.round(48 * DisplayConfig.dp)

    property int targetWidth: Math.round(parent.width - (getSideMargin() * DisplayConfig.dp))
    property int targetHeight: Math.round(parent.height - 8 * DisplayConfig.dp)

    property point minimizedPoint: Qt.point(Math.round(parent.width / 2), Math.round(parent.height - rectangleMinizedIndicator.height - (4 * DisplayConfig.dp)))
    property point centerPoint: Qt.point(Math.round(parent.width / 2), Math.round(parent.height / 2))
    property point targetPoint: Qt.point(Math.round(centerPoint.x - (targetWidth / 2)), Math.round(6 * DisplayConfig.dp))
        ...
        states: [
            ...
            State {
                ...
                PropertyChanges {
                    ...
                    radius: Math.round(2 * DisplayConfig.dp)
                }
    ...
    Item {
        ...
        anchors {
            right: rectangleMinizedIndicator.right; rightMargin: -DisplayConfig.dp
